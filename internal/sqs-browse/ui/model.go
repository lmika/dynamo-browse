...
import (
	...
	"github.com/lmika/awstools/internal/sqs-browse/ui/teamodels/messageeditview"
	"github.com/lmika/awstools/internal/sqs-browse/ui/teamodels/messagelistview"
...
type Model struct {
	...
	messageEditView   *messageeditview.Model

	//dispatcher         *dispatcher.Dispatcher
	//msgSendingHandlers *controllers.MessageSendingController
	root            tea.Model
	messageListView *messagelistview.Model
	statusAndPrompt *statusandprompt.StatusAndPrompt
}

func NewModel(commandController *commandctrl.CommandController) tea.Model {
	...
	messageListView := messagelistview.New(uiStyles)

	mainView := messageListView
	statusAndPrompt := statusandprompt.New(mainView, "Hello", uiStyles.StatusAndPrompt)
	messageEditView := messageeditview.New(statusAndPrompt)

	model := Model{
		commandController: commandController,
		root:              messageEditView,
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		if !m.statusAndPrompt.InPrompt() {
			...
			//	}
			case "e":
				return m, func() tea.Msg { return controllers.EditMessage{} }
			//case "?":
